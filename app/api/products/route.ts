...
export async function PUT(request: Request) {
  ...
  const originalExact = new RegExp(`^${safeRegex(originalSku)}import { NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { connectDB, ProductModel, CategoryModel } from '@/app/lib/mongodb';
import { EquivalentModel } from '@/app/lib/equivalents';
import { deleteBlobSafely } from '@/lib/images';
...
, 'i');
  const current = await ProductModel.findOne({ sku: originalExact }).select('_id sku mpn').lean() as unknown as { _id: unknown; sku: string; mpn?: string } | null;
  ...
  const duplicateMatchers: Record<string, unknown>[] = [
    { sku: new RegExp(`^${safeRegex(nextSku)}import { NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { connectDB, ProductModel, CategoryModel } from '@/app/lib/mongodb';
import { EquivalentModel } from '@/app/lib/equivalents';
import { deleteBlobSafely } from '@/lib/images';
...
, 'i') },
  ...
  if (nextMpn) duplicateMatchers.push({ mpn: new RegExp(`^${safeRegex(nextMpn)}import { NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { connectDB, ProductModel, CategoryModel } from '@/app/lib/mongodb';
import { EquivalentModel } from '@/app/lib/equivalents';
import { deleteBlobSafely } from '@/lib/images';
...
, 'i') });
  ...
  }).select('sku mpn name').lean() as unknown as { sku: string; mpn?: string; name?: string } | null;
  ...
  try {
    const product = await ProductModel.findByIdAndUpdate(current._id, update, { new: true, runValidators: true }).select(projection).lean();
    ...
    const duplicate = typeof error === 'object' && error !== null && 'code' in error && error.code === 11000;
    return NextResponse.json(
      { success: false, message: duplicate ? 'Another product already uses that SKU.' : 'Product could not be updated.' },
