...
export default function SmartProductManager({ onComplete, onClose }: { onComplete: () => void; onClose: () => void }) {
  ...
  const verifyAll = async () => { const aiDrafts = drafts.filter((draft) => !draft.manual); if (!aiDrafts.length) { setMessage('Manual entries do not require AI or online verification. Complete their fields, set the price, and approve.'); return; } setBusy(true); setMessage('Searching trusted engineering sources…'); let failed = 0; for (const draft of aiDrafts) if (!(await verifyOne(draft))) failed += 1; setBusy(false); setMessage(failed ? `${failed} component${failed === 1 ? '' : 's'} could not be verified. Select each red card to see the reason and retry.` : 'Verification complete. Review every card, add prices, then approve.'); };
