...
export default function SmartProductManager({ onComplete, onClose }: { onComplete: () => void; onClose: () => void }) {
  ...
  const extractList = async (file: File) => {
    ...
    try {
      ...
      if (textList) {
        ...
        setMessage(`Found ${identifiers.length} part${identifiers.length === 1 ? '' : 's'} without using AI. Review them, then use Nexar/cache lookup or AI verification as available.`);
        return;
      }
  ...
  const lookupNexarCache = async (draft: Draft, fallbackFromAI = false) => {
    ...
    if (!isKnownIdentity(identifier)) {
      setMessage('Enter a valid part number before using Nexar/cache lookup.');
      return false;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Nexar/cache lookup failed.');
      ...
      if (!localMatches.length && !alternatives.length) {
        notes.push(data.external_provider?.configured
          ? 'No local or Nexar/cache alternatives were returned for this part number.'
          : 'No cached alternatives were found. Nexar live lookup will become available after the Nexar connection is configured.');
      }
      ...
      setMessage(fallbackFromAI
        ? 'OpenAI verification is unavailable, so Nexar/cache lookup was used instead. Review and complete the card manually.'
        : 'Nexar/cache lookup complete. Review the returned reference information before approval.');
      return true;
    } catch (error) {
      const problem = error instanceof Error ? error.message : 'Nexar/cache lookup failed.';
