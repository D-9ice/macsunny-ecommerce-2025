...
export default function SmartProductManager({ onComplete, onClose }: { onComplete: () => void; onClose: () => void }) {
  ...
  const verifyOne = async (draft: Draft) => {
    ...
    try {
      ...
      setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, status: 'error', error: problem } : item));
      setMessage(problem);
      return false;
    }
  };
  const verifyAll = async () => { setBusy(true); setMessage('Searching trusted engineering sources…'); let failed = 0; for (const draft of drafts) if (!(await verifyOne(draft))) failed += 1; setBusy(false); setMessage(failed ? `${failed} component${failed === 1 ? '' : 's'} could not be verified. Select each red card to see the reason and retry.` : 'Verification complete. Review every card, add prices, then approve.'); };
