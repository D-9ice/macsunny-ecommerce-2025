...
type Draft = {
  ...
  approved: boolean; status: 'waiting' | 'checking' | 'ready' | 'error'; manual: boolean; error?: string;
};
...
const specificationsText = (specifications: Spec[]) => specifications.map((spec) => `${spec.label}: ${spec.value}`).join('\n');
const parseSpecifications = (value: string) => value.split('\n').map((line) => { const divider = line.indexOf(':'); return divider > 0 ? { label: line.slice(0, divider).trim(), value: line.slice(divider + 1).trim() } : null; }).filter((spec): spec is Spec => Boolean(spec?.label && spec.value)).slice(0, 8);
const extractManualIdentifiers = (value: string) => {
  const ignored = new Set(['part', 'parts', 'partnumber', 'part number', 'mpn', 'sku', 'quantity', 'qty', 'price', 'category', 'description', 'name']);
...
function emptyDraft(identifier: string, file?: File, manual = false): Draft {
  return { id: crypto.randomUUID(), identifier, sku: makeSku(identifier), name: identifier, manufacturer: '', category: manual ? '' : 'Uncategorized', package: '', pinCount: '', summary: '', specifications: [], datasheetUrl: '', sources: [], images: [], imageIndex: 0, localFile: file, localPreview: file ? URL.createObjectURL(file) : undefined, localImageSuitable: manual ? true : undefined, confidence: 0, verified: false, warnings: manual ? ['Entered manually by an administrator; online verification was not used.'] : [], price: '', quantity: '0', approved: false, status: manual ? 'ready' : 'waiting', manual };
}
...
export default function SmartProductManager({ onComplete, onClose }: { onComplete: () => void; onClose: () => void }) {
  ...
  const [editingOriginalSku, setEditingOriginalSku] = useState('');
  ...
  const extractList = async (file: File) => {
    ...
    try {
      ...
      const textList = file.type === 'text/csv' || file.type === 'text/plain' || lowerName.endsWith('.csv') || lowerName.endsWith('.txt');
      if (textList) {
        const identifiers = extractManualIdentifiers(await file.text());
        if (!identifiers.length) throw new Error('No recognizable component part numbers were found in this text/CSV file.');
        addIdentifiers(identifiers);
        setMessage(`Found ${identifiers.length} part${identifiers.length === 1 ? '' : 's'} without using AI. Review them, then use provider/cache lookup or AI verification as available.`);
        return;
      }
      ...
      const data = await response.json().catch(() => ({ message: response.status === 413 ? 'This upload is too large. Try a smaller document.' : 'The list service returned an invalid response.' }));
      if (!response.ok) {
        if (data.code === 'AI_CREDITS_EXHAUSTED') {
          throw new Error('AI list recognition is unavailable. Use typed entry, CSV/TXT upload, or Manual upload; those paths remain available without AI.');
        }
  ...
  const addManualPhoto = async (file: File) => {
    setBusy(true); setMessage('Preparing a manual WebP inventory card…');
    try {
      const prepared = await normalizePhoto(file);
      const draft = emptyDraft('', prepared, true);
      setDrafts((current) => [...current, draft]);
      setSelectedId(draft.id);
      setMessage('Manual card ready. Enter the product details, select a managed category, set the admin price, then approve.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'The manual product photo could not be prepared.');
    } finally { setBusy(false); }
  };

  const lookupNexarCache = async (draft: Draft, fallbackFromAI = false) => {
    const identifier = (draft.searchQuery || draft.identifier).trim();
    if (!isKnownIdentity(identifier)) {
      setMessage('Enter a valid part number before using provider/cache lookup.');
      return false;
    }

    setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, status: 'checking', error: undefined } : item));
    try {
      const response = await fetch('/api/equivalents/search', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: identifier, includeExternal: true }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'provider/cache lookup failed.');

      const source = data.cached_equivalents || data.external_equivalents;
      const alternatives = (source?.equivalents || []).slice(0, 8);
      const localMatches = data.found_in_inventory || [];
      const notes: string[] = [];

      if (localMatches.length) {
        notes.push(`MacSunny inventory match: ${localMatches.map((item: any) => item.sku).join(', ')}.`);
      }
      if (alternatives.length) {
        notes.push(`Known alternatives from ${source?.source === 'nexar' ? 'Nexar' : 'cache'}: ${alternatives.map((item: any) => item.mpn).join(', ')}.`);
      }
      if (!localMatches.length && !alternatives.length) {
        notes.push(data.external_provider?.configured
          ? 'No local, cached, Nexar, or Mouser alternatives were returned for this part number.'
          : 'No cached alternatives were found. Live external lookup requires a configured Nexar or Mouser provider.');
      }
      ...
      setDrafts((current) => current.map((item) => item.id === draft.id ? {
        ...
        error: undefined,
        warnings: [
          ...item.warnings.filter((warning) => !warning.startsWith('Nexar/cache:') && !warning.startsWith('AI verification is unavailable;')),
          ...(fallbackFromAI ? ['AI verification is unavailable; this card used the independent Nexar/cache fallback.'] : []),
          ...notes.map((note) => `Nexar/cache: ${note}`),
        ],
      } : item));

      setMessage(fallbackFromAI
        ? 'OpenAI verification is unavailable, so provider/cache lookup was used instead. Review and complete the card manually.'
        : 'provider/cache lookup complete. Review the returned reference information before approval.');
      return true;
    } catch (error) {
      const problem = error instanceof Error ? error.message : 'provider/cache lookup failed.';
      setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, status: draft.manual ? 'ready' : 'error', error: problem } : item));
      setMessage(problem);
      return false;
    }
  };
  ...
  const verifyOne = async (draft: Draft) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        if (data.code === 'AI_CREDITS_EXHAUSTED' && isKnownIdentity(draft.searchQuery || draft.identifier)) {
          return await lookupNexarCache(draft, true);
        }
        throw new Error(data.message);
      }
      ...
      setMessage(`${part.partNumber || draft.identifier} was checked against online sources.`);
      return true;
    } catch (error) {
      const problem = error instanceof Error ? error.message : 'Verification failed.';
      setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, status: draft.manual ? 'ready' : 'error', error: problem } : item));
      setMessage(problem);
      return false;
    }
  };
  const verifyAll = async () => { const aiDrafts = drafts.filter((draft) => !draft.manual); if (!aiDrafts.length) { setMessage('Manual entries do not require AI or online verification. Complete their fields, set the price, and approve.'); return; } setBusy(true); setMessage('Searching trusted engineering sources…'); let failed = 0; for (const draft of aiDrafts) if (!(await verifyOne(draft))) failed += 1; setBusy(false); setMessage(failed ? `${failed} component${failed === 1 ? '' : 's'} could not be reviewed. Select each red card to see the reason and retry.` : 'Review complete. Some cards may have used Nexar/cache fallback. Check each card, add prices, then approve.'); };
  ...
  const saveProduct = async () => {
    ...
    try {
      const response = await fetch('/api/products', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...editingProduct, originalSku: editingOriginalSku || editingProduct.sku }) }); const data = await response.json(); if (!response.ok) throw new Error(data.message);
      const savedSku = data.product?.sku || editingProduct.sku;
      if (replacementImage) { const form = new FormData(); form.append('sku', savedSku); form.append('file', replacementImage); form.append('alt', editingProduct.name); const upload = await fetch('/api/admin/products/image', { method: 'POST', body: form }); if (!upload.ok) throw new Error((await upload.json()).message); }
      setEditingProduct(null); setEditingOriginalSku(''); setReplacementImage(null); await loadProducts(); setMessage(data.renamed ? `Product SKU corrected from ${data.previousSku} to ${savedSku}. Existing image and product data were preserved.` : 'Product updated.');
